...
static int hwc_set3dmode(sun4i_hwc_context_t *ctx,int para)
{
	...
	if(ctx->hwc_layer.currenthandle)
	{
		...
		if(mode == HWC_DISP_MODE_ANAGLAGH && value == DISP_3D_SRC_MODE_SSH)
		{
			//adjust width of side-by-side picture in anaglagh mode
			layer_info.fb.size.width = (_3d_info->width + 0xF) & 0xFFFFFFF0;
		}
